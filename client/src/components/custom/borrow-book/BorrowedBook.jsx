...
function BorrowedBook() {
  ...
  const { isPending:issuePending, mutate } = useMutation({
    mutationFn: async (data) => {
      ...
      return fetch('http://127.0.0.1:5555/borrowed-books', {
        method: 'POST',
        headers: {
    ...
    onSuccess: () => {
      toast('✅ issued Book successfully!');
  ...
  if (isBooksPending || isUsersPending) return <div>Loading...</div>
  if (booksError || usersError) return <div>An error occurred: {booksError?.message || usersError?.message}</div>

  return (
    <div>
      <h2 className='my-3 text-center text-3xl'>Issue Book to Student</h2>
      ...
      <div className='flex flex-col max-w-[400px] mx-auto items-center justify-center space-y-3'>
        ...
        <Button disabled={!selection?.book || !selection?.user || issuePending} onClick={handleIssueBook}>
          Issue Book
        </Button>
      </div>
    </div>
